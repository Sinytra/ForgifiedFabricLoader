...
public final class EntrypointStorage {
	...
	private static class OldEntry implements Entry {
		...
		public synchronized <T> T getOrCreate(Class<T> type) throws Exception {
			if (object == null) {
				net.fabricmc.loader.language.LanguageAdapter adapter = (net.fabricmc.loader.language.LanguageAdapter) Class.forName(languageAdapter, true, Thread.currentThread().getContextClassLoader()).getConstructor().newInstance();
	...
	private Map<String, Collection<Object>> modInstances;
	
	public Map<String, Collection<Object>> getModInstances() {
		if (modInstances == null) {
			modInstances = new HashMap<>();
			for (List<Entry> entries : entryMap.values()) {
				for (Entry entry : entries) {
					modInstances.put(entry.getModContainer().getMetadata().getId(), entry.getInstances());
				}
			}
		}
		return modInstances;
